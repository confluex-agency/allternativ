...
import { isCaseColor, type CaseColor } from "@/lib/product-options";
...
export type CatalogVariant = {
  ...
  inStock: boolean;
  /**
   * The case this colourway comes packed in. Shown, not chosen: every
   * colourway arrives in one case colour and there are no spares. Null when
   * nobody has recorded it, and then the colourway cannot be bought.
   */
...
function toCatalogProduct(row: ProductRow): CatalogProduct {
  return {
    ...
    variants: row.variants.map((v) => ({
      ...
      caseColor: isCaseColor(v.caseColor) ? v.caseColor : null,
